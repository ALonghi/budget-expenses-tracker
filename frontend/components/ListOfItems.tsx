    ...
    return (
        <nav className="h-full overflow-y-auto" aria-label="Directory">
            {groupedMovements?.map((elems: MovementGrouped, index: number) => (
                <div key={index} className="relative rounded-lg mt-6">
                    <div
                        className="bg-slate-800 z-10 sticky top-0 border-b border-gray-200
                        px-6 py-1 text-sm font-medium text-gray-500 ">
                        <div className={`flex justify-between `}>
                            <h3 className={`text-gray-200 font-light`}>{elems.date.day === new Date().getDate()
                                ? 'Today'
                    ...
                    <ul role="list" className="relative z-0 divide-y divide-gray-400">
                        {elems.data.map((m, i) => (
                            <li key={i} className="bg-slate-800">
                                <div
